...
/**
 * Created by eljah32 on 2/28/2016.
 */
public class DeviceListFiltered extends Activity
{
        final String BLUETOOTH_DEVICE_PREFIX="HC"; //TODO the correct prefix
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (pairedDevices.size() > 0) {
            boolean no_tracker_device_fount=true;
            for (BluetoothDevice device : pairedDevices) {
                if (device.getName().startsWith(BLUETOOTH_DEVICE_PREFIX)) {
                    mPairedDevicesArrayAdapter.add(device.getName() + "\n" + device.getAddress());
                    no_tracker_device_fount=false;
                }
            };
            if (no_tracker_device_fount)
            {
                String noTrDevices = "No tracker devices found";
                mPairedDevicesArrayAdapter.add(noTrDevices);
            }
        } else {
